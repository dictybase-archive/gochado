...
func (helper *ChadoHelper) CreateCvtermId(params map[string]string) (int, error) {
	...
	tx := sqlx.MustBegin()
	result := tx.MustExec("INSERT INTO dbxref(db_id,accession) VALUES($1, $2)", dbid, xref)
	...
	result = tx.MustExec("INSERT INTO cvterm(cv_id,name,dbxref_id) VALUES($1, $2,$3)", cvid, params["cvterm"], dbxrefid)
	id64, err := result.LastInsertId()
	if err != nil {
		_ = tx.Rollback()
